...
@section('content')
    ...
    <div class="max-w-9xl mx-auto px-4 " x-data="{
        ...
        initCreate() {
            this.editMode = false;
            this.actionUrl = '{{ route('admin.tips-categories.store') }}';
            this.categoryName = '';
        ...
        initEdit(category) {
            this.editMode = true;
            this.actionUrl = `/admin/tips-categories/${category.id}`;
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
            <div class="flex flex-col gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-extrabold text-gray-900 tracking-tighter">Tips Categories</h1>
                    <p class="text-base text-gray-500 mt-1 font-medium">Manage your platform content structure and
                        visibility</p>
                </div>
            </div>
        ...
        @if (session('success'))
            <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4000)"
                class="mb-8 flex items-center p-4 text-emerald-900 bg-emerald-50 border border-emerald-100 rounded-2xl animate-fade-in-down">
                            ...
                            <tr class="group hover:bg-indigo-50/30 transition-all duration-200">
                                ...
                                <td class="px-8 py-5 text-right space-x-1">
                                    <button @click="initEdit({{ $category }})"
                                        class="inline-flex items-center px-4 py-2 text-xs font-extrabold text-indigo-600 hover:bg-indigo-600 hover:text-white rounded-xl transition-all duration-200">
                                        Edit
                                    </button>

                                    <form method="POST"
                                        action="{{ route('admin.tips-categories.destroy', $category->id) }}"
                                        class="inline-block"
                                        onsubmit="return confirm('Delete this category permanentally?')">
                                        @csrf
                                        @method('DELETE')
                                        <button
                                            class="inline-flex items-center px-4 py-2 text-xs font-extrabold text-rose-500 hover:bg-rose-500 hover:text-white rounded-xl transition-all duration-200">
                                            Delete
                                        </button>
                                    </form>
                                </td>
                            ...
                            <tr>
                                <td colspan="4" class="px-6 py-20 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <span class="text-gray-400 font-bold tracking-tight">No records found.</span>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
        ...
        <template x-teleport="body">
            <div x-show="openModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4" x-cloak>
                ...
                <div x-show="openModal" x-transition:enter="ease-out duration-300"
                    ...
                    class="relative w-full max-w-lg bg-white rounded-[2.5rem] shadow-2xl overflow-hidden border border-white/20">
                    ...
                    <div class="px-10 pt-10 pb-6 flex justify-between items-center">
                        <div>
                            <h3 class="text-2xl font-extrabold text-gray-900 tracking-tighter"
                                x-text="editMode ? 'Edit Category' : 'New Category'"></h3>
                            <p class="text-sm text-gray-500 font-medium">Please fill in the details below</p>
                        </div>
                    ...
                    transition-transform duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)]">
                                ...
                                <div class="ml-4 flex flex-col">
                                    <span
                                        class="text-sm font-extrabold text-gray-800 group-hover:text-indigo-700 transition-colors">
                                        Visible on Website
                                    </span>
                                    <span class="text-[10px] font-bold text-gray-400 uppercase tracking-tight">
                                        Enable to show in public lists
                                    </span>
                                </div>
                            </label>
                        </div>
